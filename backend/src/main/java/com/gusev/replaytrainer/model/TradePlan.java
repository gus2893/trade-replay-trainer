 ...
 * sees no edge and passes — a legitimate answer that also gets rated. Entry
 * may be MARKET (next bar's open) or a LIMIT resting at a retest price.
 ...
 */
public record TradePlan(TradeDirection direction, EntryType entryType, double limit, double stop, double target,
		String rationale, Map<String, Double> features) {

	public static TradePlan skip(String rationale, Map<String, Double> features) {
		return new TradePlan(TradeDirection.SKIP, EntryType.MARKET, 0, 0, 0, rationale, features);
	}
}
